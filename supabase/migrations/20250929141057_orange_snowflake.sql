...
CREATE POLICY "Faculty and admins can view student data" ON public.students
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Faculty can insert their own data" ON public.faculty
  FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY "Faculty can update their own data" ON public.faculty
  FOR UPDATE USING (user_id = auth.uid());

CREATE POLICY "Admins can view all faculty data" ON public.faculty
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.users 
...
-- Departments policies (read-only for most users)
...
CREATE POLICY "Faculty can view enrollments for their courses" ON public.enrollments
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Authors can delete their announcements" ON public.announcements
